...
const QUIZ_ID = 0; // <- UPDATE ME WITH QUIZ ID FROM GADGET
...
async function onSubmitHandler(evt) {
  ...
  // save to Gadget for follow up emails
          ...
          const renderedQuestions = questions.forEach((question, i) => {
            ...
            answers.forEach((answer, j) => {
              ...
              clonedSpan.insertAdjacentHTML(
                "beforeend",
                `<span><button class="button answer" id="${clonedSpan.id}" onClick=(selectAnswer(${answer.node.id},"${encodeURIComponent(
                  answer.node.text
