...
export const functionRunner = (input: string) => {
  ...
  const createInputFile = (input: string) =>
    writeFileSync(inputJsonFile, input);
  const outputByFunctionRunner = () =>
    execSync(
      `function-runner -f build/debug.wasm ${inputJsonFile} -j | jq .output.JsonOutput`
    );
