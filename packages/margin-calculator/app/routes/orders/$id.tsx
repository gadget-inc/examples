...
const margin = (price: Dinero.Dinero, cost: Dinero.Dinero) => {
  ...
  return Math.round(
    (price.subtract(cost).getAmount() / price.getAmount()) * 100
  );
};

const ShippingCostSetter = (props: {
  order: Select<
    ShopifyDraftOrder,
    { id: true; shippingCost: { amount: true; currency: true } }
  >;
}) => {
  const [popoverActive, setPopoverActive] = useState(false);
  const [shippingCost, setShippingCost] = useState(
    props.order.shippingCost
      ? String(props.order.shippingCost.amount / 100)
      : ""
  );
  ...
  const [{ data, fetching, error }, saveDraftOrder] = useAction(
    api.shopifyDraftOrder.update,
    {
  ...
  return (
    <Stack alignment="center">
      <span>
        {props.order.shippingCost
          ? Dinero(props.order.shippingCost as any).toFormat()
          : "N/A"}
      </span>
      <Popover
...
function ShippingLine(props: {
  ...
  const price = props.order.shippingLine
    ? parseShopifyMoney(props.order.shippingLine.price)
    : Dinero({ amount: 0, currency: "USD" });

  const cost =
    props.order.shippingCost && Dinero(props.order.shippingCost as any);
  Dinero({ amount: 0, currency: "USD" });
...
export default function Order() {
  ...
  const [{ data, fetching, error }] = useFindOne(
    api.shopifyDraftOrder,
    params.id!,
    {
  ...
  if (data) {
    for (const [index, { node }] of data.lineItems.edges.entries()) {
      ...
      const cost =
        node.variant?.inventoryItem &&
        parseShopifyMoney(node.variant.inventoryItem.cost!).multiply(
          node.quantity!
        );
  ...
  return (
    <Page
      title={`Margin Calculator - ${data ? data.name : "loading"}`}
      breadcrumbs={[{ content: "All Orders", url: "/" }]}
    >
      <Card sectioned>
        {fetching && <Spinner />}
        ...
        {data && (
          ...
          >
            {data.lineItems.edges.map(({ node }: any, index: number) => {
              ...
              return (
                <IndexTable.Row id={node.id} key={node.id} position={index}>
                  ...
                  <IndexTable.Cell>
                    {cost?.toFormat() ?? "not set"}
                  </IndexTable.Cell>
                  <IndexTable.Cell>{price.toFormat()}</IndexTable.Cell>
                  <IndexTable.Cell>
                    {cost ? margin(price, cost) + "%" : ""}
                  </IndexTable.Cell>
                </IndexTable.Row>
              );
            })}
            ...
            <IndexTable.Row
              id="subtotal"
              key="subtotal"
              position={data.lineItems.edges.length}
            >
              <IndexTable.Cell>
              ...
              <IndexTable.Cell>
                <TextStyle variation="strong">
                  {beforeTaxCost.toFormat()}
                </TextStyle>
              </IndexTable.Cell>
              <IndexTable.Cell>
                <TextStyle variation="strong">
                  {beforeTaxPrice.toFormat()}
                </TextStyle>
              </IndexTable.Cell>
              <IndexTable.Cell>
                <TextStyle variation="strong">
                  {margin(beforeTaxPrice, beforeTaxCost)}%
                </TextStyle>
              </IndexTable.Cell>
            </IndexTable.Row>
            <IndexTable.Row
            ...
            >
              ...
              <IndexTable.Cell>
                {data.totalPrice &&
                  parseShopifyMoney(data.totalPrice).toFormat()}
              </IndexTable.Cell>
              <IndexTable.Cell></IndexTable.Cell>
            </IndexTable.Row>
          </IndexTable>
